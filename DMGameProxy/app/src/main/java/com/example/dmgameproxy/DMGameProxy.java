...
class Person {
    ...
    // private - 如果是成员变量都需要加上私有/swift也是一样
    /*
    static - 共有属性/随着类的加载而加载/被类的所有对象共享/可以利用'类名/对象调用'
    1>.静态变量建议使用'类名调用'/也可以使用'对象调用'
    2>.非静态成员方法可以调用"静态成员方法和成员属性"
    // 与Objective-C一样（简单的说 - 1.静态方法不能访问非静态成员属性/2.静态只能访问静态）
    3>.静态成员方法不可以调用"非静态成员方法和成员属性"/因为静态成员方法优先存在于"非静态成员方法和成员属性"
    */
    /*
    '静态变量/类变量'和'成员变量/实例变量'的区别
    1.'静态变量/类变量'属于类 ｜ '成员变量/实例变量'属于对象
    2.'静态变量/类变量'存储于方法区的静态区 ｜ '成员变量/实例变量'存储于堆内存
    3.'静态变量/类变量'随着类的加载而加载，随着类的消失而消失 ｜ '成员变量/实例变量'随着对象的创建而存在，随着对象的消失而消失
    4.'静态变量/类变量'可以利用'类名/对象'调用 ｜ '成员变量/实例变量'只能通过对象调用
    */
