...
/**
 1.进程和线程的概念
 1>.进程 - 在系统中正在运行的一个应用程序/有状态（一个未运行的应用程序不是进程）/相互独立/一个应用程序可以对应多个进程/一个进程可以有多个线程（至少有一个线程/同一个进程内的线程共享进程的资源）/进程是 CPU 分配资源和调度的单位/进程类似于车间
 2>.线程 - 进程中所有的任务都是在线程中执行的/进程需要执行任务起码需要一个线程/线程是 CPU调用（执行任务）的最小单位/线程类似于工人
 */
/**
 2.串行和并行的概念
...
/**
 ...
 2>.作用 - 1>.可以适当提高程序的执行效率/2>.可以释放提高资源利用率（CPU、内存利用率）/3>.创建线程需要花费时间和内存（大量开启线程会降低程序的性能）/4>.开启线程越多 CPU 调度上的开销越大/5>.多条线程同时访问同一块数据会出现资源抢占问题
