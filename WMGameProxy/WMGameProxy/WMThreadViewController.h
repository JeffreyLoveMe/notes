...
/**
 ...
 3>.原理 - 同一时间 CPU 只能处理一条线程（只有一条线程在工作）/ 多线程并发执行其实就是 CPU 快速在多条线程之间调度切换/ 相对于 “单CPU” 而言，如果是 “多核” 就可以实现真正意义的多线程/ 1s---1000个0.001s
 4>.如果线程开启的特别多 - CPU会在N条线程之间调度，消耗大量的CPU资源/一般开启3-5条线程
 5>.作用 - ASI网络请求/SDWebImage图片下载 - 为了使界面流畅，防止界面假死
 */
/**
 4.主线程/系统默认开启/不可以手动开启
